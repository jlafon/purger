...
int
purger_filename_hash(char *in, unsigned char out[65])
{
...
int32_t
purger_crc32(const void *buf, size_t size)
{
        uint32_t crc = 0x5;
    const uint8_t *p;
...
char* 
purger_base64_decode(unsigned char * encodedtext, int len)
{
    BIO *b64, *buffer;
...
char* 
purger_base64_encode(const unsigned char * plaintext)
{
    BIO *buffer, *b64;
    BUF_MEM *buffptr;
    b64 = BIO_new(BIO_f_base64());
